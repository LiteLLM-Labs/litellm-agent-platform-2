...
async fn store_oauth_result(
    ...
    .await?;
    let existing = crate::db::managed_agents::channels::repository::get_by_kind(pool, &agent.id, "slack").await?;
    if existing.is_some() {
        crate::db::managed_agents::channels::repository::update_config(
            pool,
            &agent.id,
            "slack",
