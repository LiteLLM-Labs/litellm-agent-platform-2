...
async fn mcp_deepwiki_tool_use_across_providers() -> Result<(), Box<dyn Error>> {
    ...
    assert!(!configs.is_empty(), "set ANTHROPIC_API_KEY or CURSOR_API_KEY to run this test");
    let suffix = suffix();
    for config in &configs {
        run_for_provider(config, suffix).await?;
    }
    Ok(())
}

async fn run_for_provider(config: &ProviderConfig, suffix: u64) -> Result<(), Box<dyn Error>> {
    ...
    let agent = config.client.beta().agents().create(CreateAgentParams {
        ...
        system,
        description: None,
        tools: vec![json!({"type":"agent_toolset_20260401"}), json!({"type":"mcp_toolset","mcp_server_name":"deepwiki"})],
        mcp_servers: vec![json!({"type":"url","name":"deepwiki","url":"https://mcp.deepwiki.com/mcp"})],
        env_vars: None,
        workspace: None,
        metadata: None,
    }).await.map_err(|e| format!("[{label}] create agent: {e}"))?;
    let env = config.client.beta().environments().create(CreateEnvironmentParams {
        lap_agent_runtime: config.runtime,
        name: format!("deepwiki-mcp-env-{suffix}"),
        config: json!({"type":"cloud","networking":{"type":"unrestricted"}}),
        description: None,
        scope: None,
    }).await.map_err(|e| format!("[{label}] create environment: {e}"))?;
    let session = config.client.beta().sessions().create(CreateSessionParams {
        ...
        vault_ids: None,
        resources: None,
    }).await.map_err(|e| format!("[{label}] create session: {e}"))?;
    eprintln!("[{label}] session_id={}", session.id);
    let mut stream = config.client.beta().sessions().events().stream(&session.id).await
        .map_err(|e| format!("[{label}] open stream: {e}"))?;
    config.client.beta().sessions().events().send(&session.id, SendEventsParams {
        events: vec![json!({"type":"user.message","content":[{"type":"text","text":"Use deepwiki to get the wiki structure for the anthropics/anthropic-sdk-python repository."}]})],
    }).await.map_err(|e| format!("[{label}] send events: {e}"))?;
